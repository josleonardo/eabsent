<div x-show="{{ $drop['name'] }}" @click.away="{{ $drop['name'] }} = false"
    ...
    class="absolute right-0 z-10 mt-2 list-none rounded-sm bg-white ring-1 divide-y divide-gray-100 shadow-lg ring-black/5 focus:outline-hidden dark:bg-gray-700 dark:divide-gray-600"
    role="menu" aria-orientation="vertical" aria-labelledby="{{ $drop['label'] }}-button" tabindex="-1">
    <ul class="py-1" role="none">
        @foreach ($items as $item)
            <li>
                <a href="{{ route($item['route']) }}"
                    class="flex items-center justify-start px-4 py-2 text-sm text-gray-700 whitespace-nowrap cursor-pointer hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white"
