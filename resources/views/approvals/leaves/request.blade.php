<x-layout>
    ...
    <div class="relative overflow-x-auto shadow-md">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @if ($leaves->count() == 0)
                    <tr>
                        <td colspan="11">No leave requests to display.</td>
                    </tr>
                @endif

                @foreach ($leaves as $key => $leave)
                    <tr
                        class="{{ $leave->active != 1 ? 'bg-red-300 hover:bg-red-400 dark:bg-red-900 dark:hover:bg-red-800' : 'bg-gray-50 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700' }} border-b border-gray-200 dark:border-gray-700">
                        <th scope="row"
                            class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $leaves->firstItem() + $key }}
                        </th>
                        <td class="px-4 py-3">{{ $leave->user_id }}</td>
                        <td class="px-4 py-3">{{ $users[$leave->user_id] }}</td>
                        <td class="px-4 py-3">{{ $levels[$leave->level_id] }}</td>
                        <td class="px-4 py-3">{{ $leave->start_date }}</td>
                        <td class="px-4 py-3">{{ $leave->end_date }}</td>
                        <td class="px-4 py-3">{{ $leave->reason }}</td>
                        <td class="px-4 py-3">{{ $leave->img_path }}</td>
                        <td class="px-4 py-3">{{ $leave->created_at }}</td>
                        <td class="px-4 py-3">
                            <a href=""
    ...
    {{-- Pagination --}}
    <div class="my-4">{{ $leaves->onEachSide(2)->links() }}</div>
</x-layout>
