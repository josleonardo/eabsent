<x-layout>
    ...
    <section class="p-3 space-y-6 border border-gray-200 sm:p-4 dark:bg-gray-800 dark:border-gray-700">
        ...
        <form action="{{ route('leave-type.store') }}" method="POST" class="space-y-6 max-w-7xl">
            @csrf
            
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                <x-forms.input-field label="Type name" name="name" id="name" :isRequired="true" />
            </div>

            <div class="grid gap-4">
                <x-forms.toggle name="requires_description" :checked="true" :trueLabel="__($yesNoKey[1])" :falseLabel="__($yesNoKey[0])" />
