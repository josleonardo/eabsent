<x-layout>
    <x-slot:pageName>{{ $pageName }}</x-slot>

    <x-page-caption>
        Manage role permission where you can create, edit, and deactivate menu permission as needed.
        Each role determines what actions and menus a user can access.
    </x-page-caption>

    <x-toolbar :search="true" :create="true" :createRoute="'role-menu.create'" />

    @if ($roleMenus->isEmpty())
        <p class="text-gray-500">No role menu to display.</p>
    @else
        <div class="relative overflow-x-auto shadow-md">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-gray-700 uppercase whitespace-nowrap bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="p-4">#</th>
                        <th scope="col" class="p-4">Role</th>
                        <th scope="col" class="p-4">Menu</th>
                        <th scope="col" class="p-4">Platform</th>
                        <th scope="col" class="p-4">Active</th>
                        <th scope="col" class="p-4">Created At</th>
                        <th scope="col" class="p-4">Created By</th>
                        <th scope="col" class="p-4">Updated At</th>
                        <th scope="col" class="p-4">Updated By</th>
                        <th scope="col" class="p-4">
                            <span class="sr-only">Edit</span>
                        </th>
                    </tr>
                </thead>

                <tbody>
                    @foreach ($roleMenus as $key => $item)
                        <tr
                            class="{{ $activeKey[$item->active]['color'] ?? 'bg-gray-50 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700' }} border-b border-gray-200 dark:border-gray-700">
                            <th scope="row"
                                class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $roleMenus->firstItem() + $key }}
                            </th>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $item->role_name }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $item->menu_name }}</td>
                            <td class="px-4 py-3">
                                {{ $platforms[$item->platform] ? __($platforms[$item->platform]) : __('Unknown') }}</td>
                            <td class="px-4 py-3 text-center">
                                {{ $yesNoKey[$item->active] ? __($yesNoKey[$item->active]) : __('Unknown') }}</td>
                            <td class="px-4 py-3">{{ $item->created_at }}</td>
                            <td class="px-4 py-3 text-center">{{ $item->created_by }}</td>
                            <td class="px-4 py-3">{{ $item->updated_at }}</td>
                            <td class="px-4 py-3 text-center">{{ $item->updated_by }}</td>
                            <td class="px-4 py-3 text-center">
                                <a href="{{ route('role-menu.edit', [$item->role_id, $item->menu_id]) }}"
                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>

        <div class="my-4">{{ $roleMenus->onEachSide(2)->links() }}</div>
    @endif
</x-layout>
